<template>
  <v-layout column>
    <v-container>
      ...
      <v-btn depressed block :color="colors[ani]" >
       {{ani}}: {{ an }}
      </v-btn>
        </v-col>
      </v-row>
    ...
    >
      ...
      <template v-slot:item.form="{item}">
        <td v-if="item.form" style="white-space:nowrap">
          <v-btn outlined small :to="`/app/services/forms?id=${item.id}`">{{item.form.id}}</v-btn>
				</td>
      </template>
      <!-- eslint-disable-next-line -->
...
export default {
  ...
  data: () => ({
    serviceHeaders:[
      ...
      {text:'Execution Date', value:'serviced_on'},
  ...
  methods: {
    ...
    countServices(){
      this.$axios.get('/auth/analytics/serviceCount').then(
        (res) => {
