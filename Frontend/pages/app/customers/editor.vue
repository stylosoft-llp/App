<template>
  <v-layout container fill-height align-center justify-center>
    <v-card width="100%" max-width="600">
      <v-card-title
        >{{$route.query.id?'Edit':'Add New'}} Customer
        <v-spacer></v-spacer>
      ...
      <v-container>
        ...
        <div v-if="userDetail">
          <v-text-field
            ...
            dense
          />
          <h3 class="my-1">Primary Address</h3>
          <v-text-field v-model="userDetail.company_name" label="User Company" outlined dense />
          <v-text-field v-model="userDetail.team" label="User team" outlined dense />
          ...
          <v-text-field
            ...
            dense
          />
          <h4 class="mb-2">Contact Person 1 (Consent Authority)</h4>
          <v-row dense>
            <v-col>
              <v-text-field
          ...
          <h4 class="mb-2">Contact Person 2 (Consent Authority)</h4>
          <v-row dense>
            <v-col>
              <v-text-field
          ...
          <v-card-title
            >Additional Address Blocks <v-spacer />
            <v-btn outlined @click="addAddress()">Add Block</v-btn>
          </v-card-title>
          <div v-for="(addr, addi) in userDetail.addresses" :key="addi">
            <v-text-field
              outlined dense
              v-model="addr.zone"
              label="Zone"
              append-icon="mdi-delete"
              @click:append="userDetail.addresses.splice(addi,1)"
            ></v-text-field>
            <v-textarea
              outlined rows="2"
  ...
  methods: {
    getUser() {
      this.$axios
        ...
        .then((res) => {
          this.user = res.data;
          res.data.user_detail ? (this.userDetail = res.data.user_detail) : "";
        });
    },
    addUser() {
      let user = this.user;
    ...
    addAddress() {
      ...
      if(!this.userDetail.addresses){
        this.userDetail.addresses = [];
      }
      this.userDetail.addresses.push({
        zone: "",
        address: "",
      });
    },
