    ...
    <v-card width="100%">
      <v-card-text>
        ...
        <v-container>
          ...
          <v-btn v-if="!service.time_location || !service.time_location.start" @click="addRecord()" color="primary" :loading="geoLocating">
          ...
          <v-simple-table>
            ...
            <tbody v-if="service.time_location">
              ...
              <tr v-if="service.time_location.end">
                ...
                <td><a target="_blank" :href="`https://maps.google.com/?q=${service.time_location.end.coordinates.latitude},${service.time_location.end.coordinates.longitude}`">{{service.time_location.end.coordinates && 'Latitude:'+service.time_location.end.coordinates.latitude}} <br/> {{'Longitude: ' + service.time_location.end.coordinates.longitude}}
                </a></td>
              </tr>
            </tbody>
  ...
  methods: {
    ...
    updateService() {
      ...
      this.$axios
        ...
        .then(
          ...
          (err) => {
            this.$toast.error("Something Went Wrong",{icon:'mdi-alert'})
    ...
    addRecord(v) {
      ...
      navigator.geolocation.getCurrentPosition(({ coords }) => {
        this.$toast.success("Time and Location Fetched");
        const coordinates = {
    ...
    geoCode(longitude, latitude) {
      ...
      this.$axios
        ...
        .then(
          ...
          (err) => {
            this.$toast.console.warning("Unable To Fetch Location From Map Service!");
          }
        );
    }, //GEOCODE()
  },
