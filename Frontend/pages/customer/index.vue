<template>
  ...
  <div class="my-2 d-flex flex-row align-start justify-center">
    <v-text-field label="Search Tickets"
...
<!--                 <v-menu
    ...
    <v-btn @click="fetchServices()" class="mx-1" color="accent">
      <v-icon>mdi-magnify</v-icon>
        ...
        <v-btn to="/customer/newTicket" color="primary" class="mx-1">
    <v-icon left>mdi-plus-box-multiple</v-icon> Raise Ticket
    </v-btn>
  </div>
    <v-data-table
      :show-select="false"
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <div class="row">
<!--         <v-btn :to="'/customer/newTicket?id='+item.id" color="blue" icon small>
...
export default {
  ...
  data: () => ({
    serviceHeaders:[
      ...
      {text:'Address', value:'customer.address_1'},
  ...
  methods: {
    ...
    async fetchServices() {
      this.loading=true;
      await this.$axios.get('auth/tickets',{params:{
